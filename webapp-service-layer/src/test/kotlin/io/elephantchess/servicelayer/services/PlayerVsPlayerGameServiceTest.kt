...
class PlayerVsPlayerGameServiceTest : ServiceTest() {
    ...
    @Nested
    @DisplayName("Play move behavior")
    inner class PlayMoveBehavior {

        @Test
        fun `completes a rated xiangqi game and updates only xiangqi ratings`() = runTest {
        ...
        @Test
        fun `completes a rated manchu game and updates only manchu ratings`() = runTest {
        ...
        @Test
        fun `rejects playing opponent color non-parsable move and illegal move`() = runTest {
            val gameId = createAndJoinGame(userId1, userId2, inviterColor = RED)
         ...
         */
        @Test
        fun `rejects move that is illegal despite prior valid move history`() = runTest {
         ...
         */
        @Test
        fun `detects perpetual checking at expected move`() = runTest {
            val moves = listOf(
    ...
    @Nested
    @DisplayName("Game creation limit")
    inner class GameCreationLimitBehavior {

        /**
         * User should not be able to create more than 3 CREATED PvP games with the same settings.
         */
        @Test
        fun `rejects creating more than 3 pending games with same settings`() = runTest {
         ...
         */
        @Test
        fun `applies limit per time category`() = runTest {
            val request = CreateGameRequest(
                inviterColor = RED,
                isRated = true,
                timeControlBase = 30.minutes.inWholeSeconds.toInt(),
