...
class BoardGui {
    ...
    /**
     * Draw the position described by {@code targetFen}.
     *
     ...
     */
    drawHistoricalPosition(targetFen, animate = true) {
        if (this.isInPlaceHolderMode()) {
            console.warn('Can not draw historical position when in placeholder mode');
            return;
        }

        if (animate) {
            this.#drawHistoricalPositionAnimated(targetFen);
        } else {
            this.#drawHistoricalPositionNoAnimation(targetFen);
        }
    }

    #drawHistoricalPositionNoAnimation(targetFen) {
        this.clearBoard();
        this.disablePlayerMove();
        this.#board.loadFen(targetFen);
        const isPlayerEnabled = this.#isPlayerMoveEnabled;
     ...
     * @param targetFen {string}
     */
    #drawHistoricalPositionAnimated(targetFen) {
        this.#hideAllPiecePlaceHolders();
        this.#hideHighlightedLastMove();

        // capture state BEFORE disabling the player so we can restore it afterwards
        const wasPlayerEnabled = this.#isPlayerMoveEnabled;
        this.disablePlayerMove();
    ...
    updateHighlightedChecks() {
        ...
        if (!this.#options.mini || this.#options.forceRenderChecks) {
            let redGeneral = this.#board.findGeneral(Color.RED);
            let blackGeneral = this.#board.findGeneral(Color.BLACK);
